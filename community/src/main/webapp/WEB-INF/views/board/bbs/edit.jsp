...
<div class="container py-5">
        <form name="writeform" class="writeform" action="write" method="post" onsubmit="return validateForm();">
            <div class="row">
                ...
                <input type="text" name="writer" class="col-3 form-control mb-3" placeholder="작성자" value="${bbs.writer }" />
                <c:choose>
				...
				<c:if test="${badmin.category == 1 }">
					...
					<select name="category" class="col-3 form-control mb-3">
						...
						<c:forEach var="category" items="${categories }">
							<option value="${category.categoryText }"<c:if test="${category.categoryText == bbs.category }"></c:if>>${category.categoryText }</option>
						</c:forEach>
					</select>
					<div class="col-8"></div>
                 ...
                 <div class="col-11 mb-3 p-0">
                    <textarea id="content" name="content" >${bbs.content }</textarea>
                 </div>
                 <c:if test="${badmin.sec == 1 }">
	                 <div class="col-12 text-right">
	                 	<label><input type="checkbox" name="sec" value="1" <c:if test="${bbs.sec == 1}">checked</c:if> /> 비밀글</label>
	                 </div>
                 </c:if>
            </div>
            <c:if test="${user.userid == null || user.userid.isEmpty() }">
            	<div class="g-recaptcha my-4 d-flex justify-content-end" data-sitekey="6LcuGQcrAAAAAAJvCPZ9K7-QAHmSiHejL5CeQHee"></div>
            </c:if>
