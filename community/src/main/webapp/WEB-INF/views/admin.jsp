            ...
            <div class="modal" id="cate-${list.id }">
               <div class="modal-dialog">
                   <div class="modal-content" style="padding:30;">
                       ...
                       <c:if test="${not empty list.boardCategory }">
                           <button type="button" data-bid="${list.id }" class="btn btn-warning categoryEdit">수정</button> 
                           </form>
                        </c:if>   
						...
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header">
									<h5 class="modal-title">파일 업로드 설정</h5>
								...
								<div class="modal-body p-5">
									<form id="edtform-${list.id }" method="post">
			        					<p class="text-danger">*단위: MB(메가바이트)</p>
			        					<label class="mt-2">파일 최대 크기</label>
			        					<input type="hidden" name="bid" value="${list.id }" />
			        					<div class="input-group mb-3">
			        						<input type="text" name="filesize" class="form-control" placeholder="파일 최대 크기(단위(MB))" value="${list.filesize }" />
			        					</div>
			        					<label class="mt-2">전체 파일 최대 크기</label>
			        					<div class="input-group mb-3">
			        						<input type="text" name="allfilesize" class="form-control" placeholder="파일 최대 크기(단위(MB))" value="${list.allfilesize }" />
			        					</div>
			        					<label class="mt-2">확장자 제한</label>
			        					<div class="input-group mb-3">
				        					<label class="mr-4"><input type="radio" name="filechar" value="0" <c:if test="${list.filechar == 0}">checked</c:if>/>업로드 모두 허용</label>
				        					<label><input type="radio" name="filechar" value="1" <c:if test="${list.filechar == 1}">checked</c:if> />이미지 전용</label>
			        					</div>
			        					<div class="thumbnail">
			        						<label class="mt-2">기본 썸네일 크기</label>
...
$(function(){
	...
	$(".del").click(function(){
		if(confirm("정말로 삭제하시겠습니까? 삭제하면 복구가 불가능합니다.")){
